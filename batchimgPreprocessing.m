function result = batchimgPreprocessing(inDir, outDir)
%batchimgPreprocessing Batch process images using imgPreprocessing
% RESULT = batchimgPreprocessing(INDIR, OUTDIR) processes each file in INDIR
% using the function imgPreprocessing.
%
% The following fields from the output of imgPreprocessing are written with their
% corresponding file format to the output directory OUTDIR:
%    output saved as png format
%
% The following fields are returned in the struct array RESULT
%
%
...
if(nargin<1)
    inDir = 'C:\Users\geoffrey\Documents\Matlab\handgesture\dataset5\student3\pose1';
end
if(nargin<2)
    outDir = sprintf('%s/%s',inDir,'preprocessed');
    mkdir(outDir);
end
...
% Process each image using imgPreprocessing
for imgInd = 1:numel(imds.Files)
    ...
    try
        ...
        % Process
        oneResult = struct();oneResult.output = imgPreprocessing(im);
