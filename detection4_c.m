function [class,score,outImg]=detection4_c(src,learners,filters,smoothFilter,colorSpace,...
...
%     [features,~,featColor,featShape]=getFeatureVector(dataColor,dataEdge,cellSize,blockSize,blockOverlap,src);
    gaborVec=extract_gaborFeature(src,36);
%     featuresArr={featColor;featShape;gaborVec};
%     disp(gaborVec);
    featuresArr={gaborVec};
    class=-1;
    labelName='';
    for i=1:lnSize
        learner=learners{i};
        [tclass,score]=predict(learner,gaborVec,'BinaryLoss','binodeviance','PosteriorMethod','gp');
        if(tclass~=-1)
            if(class==-1)
                class=tclass;
            elseif(class~=tclass)
    ...
    if(class~=-1)
        labelName=sprintf('pose %d(%f|%f)',class,score(tclass)*100,score(class)*100);
    end
%     if(class==1)
