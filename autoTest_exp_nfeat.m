...
global m_svmLabels m_svmVector m_svmVector2 m_svmLabels2 m_svmVector_gabor m_svmLabel_gabor;
...
for a=1:size(labels,2)
        ...
        while(m<=size(files,2))
            ...
            gfeat=extract_gaborFeature(sample);
...
%     disp(folderName);
    ...
    while(m<=size(files,2))
        sample=imread(files{m});
        sample=imresize(sample,[200 200]);
        [output,score,img]=detection4(sample,{m_ssvm m_ssvm2 m_ssvm3},m_filter,m_sfilter,m_cform,...
            m_cellSize,m_blockSize,m_blockOverlap);
