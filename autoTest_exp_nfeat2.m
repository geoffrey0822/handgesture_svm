...
trainConf={
           './conf/dumpTest2/setting_2c_right.conf'
           './conf/dumpTest2/setting_2c_front.conf'};
       
testConf={
    './conf/dumpTest2/test/test_2c_right.conf'
    './conf/dumpTest2/test/test_2c_front.conf'};
...
% % % % 
% % % % trainConf={
% % % %            './conf/dumpTest2/setting_1c_front.conf'
% % % %            './conf/dumpTest2/setting_1c_left.conf'
% % % %            './conf/dumpTest2/setting_1c_right.conf'
% % % %            './conf/dumpTest2/setting_2c_front.conf'
% % % %            './conf/dumpTest2/setting_2c_left.conf'
% % % %            './conf/dumpTest2/setting_2c_right.conf'
% % % %            './conf/dumpTest2/setting_3c_front.conf'
% % % %            './conf/dumpTest2/setting_3c_left.conf'
% % % %            './conf/dumpTest2/setting_3c_right.conf'};
% % % %        
% % % % testConf={
% % % %     './conf/dumpTest2/test/test_1c_front.conf'
% % % %     './conf/dumpTest2/test/test_1c_left.conf'
% % % %     './conf/dumpTest2/test/test_1c_right.conf'
% % % %     './conf/dumpTest2/test/test_2c_front.conf'
% % % %     './conf/dumpTest2/test/test_2c_left.conf'
% % % %     './conf/dumpTest2/test/test_2c_right.conf'
% % % %     './conf/dumpTest2/test/test_3c_front.conf'
% % % %     './conf/dumpTest2/test/test_3c_left.conf'
% % % %     './conf/dumpTest2/test/test_3c_right.conf'};
...
for testSet=1:size(trainConf,1)
    ...
    if ~exist(featFile)
        [edgeLabels,edges,hogLabels,hogVec,gaborLabels,gaborVec]=extractTrainSample(trainConf{testSet});
        save(featFile,'edgeLabels','edges','hogLabels','hogVec','gaborLabels','gaborVec');
    else
...
global m_svmLabels m_svmVector m_svmVector2 m_svmLabels2 m_svmVector_gabor m_svmLabel_gabor m_svmVector3 m_svmLabels3;
...
m_svmLabel_gabor=S.gaborLabels;
m_svmVector_gabor=S.gaborVec;
m_svmVector3=m_svmVector_gabor;
m_svmLabels3=m_svmLabel_gabor;
...
m_ropts.BoxConstraint=8.999214;
...
function initParams()
    ...
    global dataset h1 h2 h2t hc hct tmpDir svm svm2 svm3 m_ssvm m_ssvm2 m_ssvm3...
    ...
    m_opts.KernelOffset=1e-2;
    m_opts.KernelScale=3;
    
    m_classNames={};
    m_sampleSize=[227 227];
    m_cellSize=[16 16];
    m_blockSize=[4 4];
    ...
    svm3=templateSVM('Standardize',0,'KernelFunction','rbf',...
    'GapTolerance',m_opts.GapTolerance,'BoxConstraint',m_opts.BoxConstraint,'OutlierFraction',m_opts.OutlierFraction,...
    'KernelOffset',m_opts.KernelOffset,'KernelScale',m_opts.KernelScale,...
    'DeltaGradientTolerance',1e-8,'IterationLimit',1e6,...
    'KKTTolerance',1e-8,'Nu',.1);
...
function randomizeParams(idx)
    ...
    global dataset h1 h2 h2t hc hct tmpDir svm svm2 svm3 m_ssvm m_ssvm2...
    positives negatives detector capDir rawImg...
    m_showAsRaw m_filter m_sfilter m_cform m_sampleStarted...
    m_svmVector m_svmLabels m_svmVector2 m_svmLabels2 m_classNames  ...
...
function [edgeLabel,edges,hogLabel,hogVec,gaborLabel,gaborVec]=extractTrainSample(file)

global m_svmLabels m_svmVector m_svmVector2 m_svmLabels2 m_svmVector3 m_svmLabels3 m_classNames m_cform m_sfilter m_filter ...
...
for a=1:size(labels,2)
        ...
        while(m<=size(files,2))
            ...
            sample=imresize(sample,[227 227]);
...
%             featShape=[];
    ...
    m_svmVector=hogVec;
    m_svmVector2=edges;
    m_svmLabels3=gaborLabel;
    m_svmVector3=gaborVec;

end

function trainMySVM(file)
    global svm svm2 m_svmLabels m_svmVector m_svmLabels2 m_svmVector2 m_svmLabels3 m_svmVector3...
    tmpDir m_cform m_sfilter m_filter m_ssvm m_ssvm2 m_ssvm3 m_classNames;
    if(size(m_svmLabels3,1)==0||size(m_svmVector3,1)==0)
    ...
    if ~exist(file)
        ...
        'Coding','allpairs','Options',statset('UseParallel',0),...
        'Prior','empirical');
        m_ssvm3=fitcecoc(m_svmVector3,m_svmLabels3,'Learners',svm,...
        'Coding','allpairs','Options',statset('UseParallel',0),...
        'Prior','empirical');
        save(file,'m_ssvm','m_ssvm2','m_ssvm3');
    else
        data=load(file);
        m_ssvm=data.m_ssvm;
        m_ssvm2=data.m_ssvm2;
        m_ssvm3=data.m_ssvm3;
    end
end
...
global m_opts svm m_svmLabels m_svmVector tmpDir m_cform m_sfilter m_filter m_ssvm m_classNames ...
    m_cellSize m_blockSize m_blockOverlap m_ssvm2 m_ssvm3 predictLabels observedLabels classes classMap all_target all_observed;
...
%     disp(folderName);
    ...
    while(m<=size(files,2))
        sample=imread(files{m});
        sample=imresize(sample,[227 227]);
        [output,score,img]=detection4_c(sample,{m_ssvm3},m_filter,m_sfilter,m_cform,...
            m_cellSize,m_blockSize,m_blockOverlap);
